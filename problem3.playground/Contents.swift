...
let array:[String] = ["sum", "division", "difference",  "complement", "power", "multiplication"]
//size is between 5 and 10
...
func devision(_ a: Int, _ b: Int) ->Int {
    if b == 0 {
    return nil
...
var dictionary:[String: (Int, Int) -> Double] = [:]
    dictionary["sum"] = sum
    dictionary["division"] = devision_
    dictionary["difference"] = difference
    dictionary["power"] = power
    dictionary["multiplication"] = multiplication
    
func caluculator(_ num1: Int, _ num2: Int, out: (Int, Int) -> (Double, (Int, Int) -> Double)?{
    
    if dictionary.keys,contains(opt) {
        let operation = dictionary[opt] {
return (result, )_
